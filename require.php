<?php
namespace COI\Social;

include(__DIR__.'/classes/COI/Social/AbstractElement.php');
include(__DIR__.'/classes/COI/Social/Facebook.php');
include(__DIR__.'/classes/COI/Social/Twitter.php');
include(__DIR__.'/classes/COI/Social/LinkedIn.php');
include(__DIR__.'/classes/COI/Social/GooglePlus.php');
include(__DIR__.'/classes/COI/Social/Flattr.php');
include(__DIR__.'/classes/COI/Social/StumbleUpon.php');

include(__DIR__.'/classes/COI/Social/Manager.php');